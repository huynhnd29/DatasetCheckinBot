...
async function accessSpreadSheet(name, user_Name, hour, date, chat_id, is_late){
    ...
    await doc.useServiceAccountAuth({
      ...
      var late;
      if(is_late) late = "x";
      firstSheet.addRow({Name: name, userName: user_Name, Time: hour, Date: date, Late: late});
      if(chat_id == "-1001193572016"){
          anhSheet.addRow({Name: name, userName: user_Name, Time: hour, Date: date, Late: late});
      }
      if(chat_id == "-1001168450211"){
        longSheet.addRow({Name: name, userName: user_Name, Time: hour, Date: date, Late: late});
      }
      if(chat_id == "-1001453548823"){
        binhSheet.addRow({Name: name, userName: user_Name, Time: hour, Date: date, Late: late});
      }
}
...
bot.command('diemdanh', ctx => {
    ...
    bot.telegram.sendMessage(ctx.chat.id, 'Xin chào ' + name + '! Bạn đã điểm danh vào lúc ' + hour + " " + date, {});
